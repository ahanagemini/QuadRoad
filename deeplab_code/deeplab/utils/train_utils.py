...
def dice_coe(output, target, loss_type='sorensen', axis=0, smooth=1e-5):
...
def add_dice_loss_for_each_scale(scales_to_logits,
                                                  labels,
                                                  num_classes,
                                                  ignore_label,
                                                  loss_weight=1.0,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    if top_k_percent_pixels == 1.0:
      ...
      outputs = tf.nn.softmax(tf.reshape(logits, shape=[-1, num_classes]))
      dice_loss = 1 - dice_coe(outputs, one_hot_labels)
      ...
      print("Error: Loss not supported")

...
def predict_labels(images, purpose, label_no, model_options, image_pyramid=None):
  """Predicts segmentation labels.
  ...
  Args:
    ...
    purpose: is whether to get the class prediction or the softmax of label_no
    label_no: label whose softmax is stored
  Returns:
    A dictionary with keys specifying the output_type (e.g., semantic
      prediction) and values storing Tensors representing predictions (argmax
      over channels). Each prediction has size [batch, height, width].
  ...
  for output in sorted(outputs_to_scales_to_logits):
    ...
    if model_options.prediction_with_upsampled_logits:
      logits = _resize_bilinear(logits,
                                tf.shape(images)[1:3],
                                scales_to_logits[MERGED_LOGITS_SCOPE].dtype)
      if purpose == 'eval': # if we want to save the class prediction
          predictions[output] = tf.argmax(logits, 3)
      
      elif purpose == 'vis': # if we want to save the softmax values
          softmax = tf.nn.softmax(logits)
          softmax_as_image = tf.multiply(softmax, 200.0)
          predictions[output] = softmax_as_image[0:,0:,0:,label_no]

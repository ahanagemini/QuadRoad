...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    if tuple(FLAGS.eval_scales) == (1.0,):
      ...
      predictions = model.predict_labels(samples[common.IMAGE], 'eval', 0, model_options,
    ...
    for eval_scale in FLAGS.eval_scales:
      predictions_tag += '_' + str(eval_scale)
      fp_tag += '_' + str(eval_scale)
      fn_tag += '_' + str(eval_scale)
      tp_tag += '_' + str(eval_scale)
      tn_tag += '_' + str(eval_scale)
    ...
    # Define the evaluation metric.
    metric_map = {}
    #indices = tf.squeeze(tf.where(tf.less_equal(labels, dataset.num_of_classes - 1)), 1)  # ignore all labels >= num_classes
    #labels = tf.cast(tf.gather(labels, indices), tf.int32)
    #print(dataset.ignore_label)
    #predictions = tf.gather(predictions, indices)
    metric_map[predictions_tag] = tf.metrics.mean_iou(
        predictions, labels, dataset.num_of_classes, weights=weights)

    metric_map[fp_tag] = tf.metrics.false_positives(
        predictions, labels, weights=weights)
    metric_map[fn_tag] = tf.metrics.false_negatives(
        predictions, labels, weights=weights)
    metric_map[tp_tag] = tf.metrics.true_positives(
        predictions, labels, weights=weights)
    metric_map[tn_tag] = tf.metrics.true_negatives(
        predictions, labels, weights=weights)
